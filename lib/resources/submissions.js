...
const { success, xml, isFalse, isTrue, contentDisposition, redirect, withEtag, convertToSubmissionVersion } = require('../util/http');
...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  const openRosaSubmission = (path, _endpoint, draft, getForm) => {
    ...
    // Nonstandard REST; OpenRosa-specific API.
    service.post(path, multipart.any(), multerUtf, multipartErrorHandler, _endpoint.openRosa(({ Forms, Submissions, SubmissionAttachments }, { params, files, auth, query, userAgent, headers }) =>
            ...
            return (deprecatedId != null)
              ...
              : Submissions.getAnyDefByFormAndInstanceId(form.id, partial.instanceId, draft)
                .then((maybeExtant) => {
                  ...
                  return Promise.all([
                    Submissions.createNew(partial, form, query.deviceID, userAgent, headers[ODK_CLIENT]),
                    Forms.getBinaryFields(form.def.id)
                  ])
  ...
  const restSubmission = (base, draft, getForm) => {
    ...
    service.use(express.raw({ type: ['application/xml', 'text/xml'], limit: '100mb' }));
    service.post(`${base}/submissions`, endpoint(async ({ Forms, Submissions, SubmissionAttachments, db }, { params, auth, query, userAgent }, request) => {

      const acceptedContentTypes = new Set(['application/xml', 'text/xml']);
      if (!acceptedContentTypes.has(request.get('Content-Type'))) throw Problem.user.invalidContentType(Array.from(acceptedContentTypes).join(', '));

      const form = await Forms.getByProjectAndXmlFormId(params.projectId, params.xmlFormId, Form.WithoutDef);
      const actorId = auth.actor.get().id;
      const formId = form.get().id;
      const bodyxml = await request.body.toString('UTF-8');

      // if (!auth.can('submission.create', form)) throw new Error('TODO');

      return db.one(sql`
        WITH

          xmlized_submission AS (
            SELECT ${bodyxml}::xml AS xml
          ),
          ...
          submission_details AS (
            SELECT
                xmltable.*
            FROM
                xmlized_submission sub,
                xmltable(
                  '/*[1]'
                  PASSING (sub.xml)
                  COLUMNS
                    xml_form_id text PATH '@id',
                    xml_form_version text PATH '@version',
                    instance_id text PATH '/meta/instanceId',
                    deprecates text PATH '/meta/deprecatedId',
                    device_id text PATH '/meta/deviceId',
                    instance_name text PATH '/meta/instanceName',

                    is_encrypted text PATH '@encrypted',
                    local_key text PATH '/base64EncryptedKey',
                    signature text PATH '/base64EncryptedElementSignature',
                    enc_data_attachment_name text PATH '/encryptedXmlFile'
                )
          ),

          actual_instanceid AS (
            SELECT coalesce(
              instance_id,
              format('uuid:%s', gen_random_uuid())
            ) as iid
            FROM
              submission_details
          ),
          ...
          targeted_formdef AS (
            ...
            FROM
              ...
              INNER JOIN forms ON (
                ...
                forms.id = ${formId}
                AND
                forms.state IN ('open', 'closing')
                AND
                forms."deletedAt" IS NULL
              )
            WHERE
              forms."xmlFormId" = submission_details.xml_form_id
              AND
              fdef.version = submission_details.xml_form_version
            ORDER BY fdef."publishedAt" DESC NULLS LAST LIMIT 1 -- pick a published one over a draft one
          ),
          ...
          created_submission AS (
            ...
            VALUES (
              DEFAULT,
              ${formId},
              (select iid from actual_instanceid),
              now(),
              NULL,
              NULL,
              ${actorId},
              (select device_id from submission_details),
              false,
              NULL
            )
            RETURNING *
          ...
          created_submission_def AS (
            INSERT INTO submission_defs (
              ...
              (select xml from xmlized_submission),
              (select id from targeted_formdef),
              ${actorId},
              now(),
              (SELECT CASE WHEN is_encrypted = 'yes' THEN enc_data_attachment_name END from submission_details),
              (SELECT CASE WHEN is_encrypted = 'yes' THEN local_key END from submission_details),
              (SELECT CASE WHEN is_encrypted = 'yes' THEN signature END from submission_details),
              true,
              (select instance_name from submission_details),
              (select iid from actual_instanceid),
              ${userAgent},
              (select device_id from submission_details),
              true
          ...
          mentioned_attachment_filenames AS (
            SELECT
              unnest(xpath(format('/*[1]%s/text()', ff.path), xs.xml)) as the_attname
            FROM form_fields ff, xmlized_submission xs
            WHERE (
              ff."schemaId" = (select schema_id from targeted_formdef)
              AND
              ff.type = 'binary'
            )
          ),

          created_attachment_filenames AS (
            INSERT INTO submission_attachments (
              "blobId",
              name,
              "submissionDefId",
              index, -- TODO what's this
              "isClientAudit" -- TODO what's this
            )
            SELECT
              NULL,
              maf.the_attname,
              (select id from created_submission_def),
              NULL,
              false
            FROM mentioned_attachment_filenames maf
          )

          select * from created_submission_def; -- TODO match output
          `);

    }));

    service.post(`${base}/submissions`, endpoint(({ Forms, Submissions, SubmissionAttachments, db }, { params, auth, query, userAgent }, request) =>
    ...
    service.put(`${base}/submissions/:instanceId`, endpoint(({ Forms, Submissions, SubmissionAttachments }, { params, auth, query, userAgent, headers }, request) =>
  ...
  service.post(`/test/:key/projects/:projectId/forms/:xmlFormId/draft/submissions`, anonymousEndpoint(({ Forms, Submissions, SubmissionAttachments }, { params, query, userAgent, headers }, request) =>
    Submission.fromXml(request)
  ...
  const dataOutputs = (base, draft, getForm) => {
    ...
    const fullzip = async ({ Audits, ClientAudits, Keys, Forms, SubmissionAttachments, Submissions },
      auth, params, query, passphrases, response) => {
      const form = await getForm(params, Forms).then((theForm) => auth.canOrReject('submission.read', theForm));
      const options = QueryOptions.fromSubmissionCsvRequest(query);
      const submissionsEtag = await Submissions.getODataSelectionEtag(form.id, draft, options);
    ...
    const csv = async ({ Audits, Keys, Forms, Submissions },
      auth, params, query, passphrases, response, rootOnly) => {
      const form = await getForm(params, Forms).then(foundForm => auth.canOrReject('submission.read', foundForm));
      const options = QueryOptions.fromSubmissionCsvRequest(query);
      const submissionsEtag = await Submissions.getSelectionEtag(form.id, draft, options.filter);

      const generateResponse = () => Promise.all([
        (options.deletedFields === true) ? Forms.getMergedFields(form.id) : Forms.getFields(form.def.id),
      ...
      return withEtag(submissionsEtag, generateResponse, true);
    };
    ...
    service.get(base, endpoint(async ({ Forms, Submissions }, { params, auth, queryOptions }) => {
      const form = await getForm(params, Forms).then((theForm) => auth.canOrReject('submission.list', theForm));
      const odataDeletedFilter = isTrue(queryOptions.argData.deleted) ? 'not __system/deletedAt eq null' : '__system/deletedAt eq null';
      const submissionsEtag = await Submissions.getSelectionEtag(form.id, draft, odataDeletedFilter, Number.parseInt(queryOptions.argData.limit, 10) || null, Number.parseInt(queryOptions.argData.offset, 10) || null);
      const generateBody = () => Submissions.getAllForFormByIds(params.projectId, form.xmlFormId, draft, queryOptions);
      return withEtag(submissionsEtag, generateBody, true);
    }));
        ...
        .then(([ form, maybeSub ]) => Promise.all([
          ...
          maybeSub.map(resolve).orElseGet(() =>
            Submissions.getRootForInstanceId(form.id, params.instanceId, draft)
              .then(getOrNotFound)
              .then((rootId) => {
                redirect(convertToSubmissionVersion(originalUrl, params.instanceId, rootId));
              }))
        ]));
    ...
    service.get(`${base}/:instanceId/attachments/:name`, endpoint(({ s3, Forms, Submissions, SubmissionAttachments }, context) =>
      getOrRedirect(Forms, Submissions, context)
        ...
        .then(getOrNotFound)
        .then((blob) => blobResponse(s3, context.params.name, blob, true))));
    ...
    service.get(`${base}/:rootId/versions/:instanceId/attachments/:name`, endpoint(({ s3, Forms, Submissions, SubmissionAttachments }, { params, auth }) =>
      getForm(params, Forms)
        ...
        .then(([ blob ]) => blobResponse(s3, params.name, blob, true))));
