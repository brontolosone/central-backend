...
const withEtag = (serverEtag, fn, weak=true) => (request, response) => {

  response.set('ETag', `${weak ? 'W/': ''}"${serverEtag}"`);
  ...
  if ([`"${serverEtag}"`, `W/"${serverEtag}"`].includes(clientEtag)) { // take into account that nginx weakens the ETag when downstream a transport-encoding different from "ours" is used
    response.status(304);
    return;
  }
  return fn();
