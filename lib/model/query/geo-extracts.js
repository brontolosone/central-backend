...
const hasElements = (fnarg) => (Array.isArray(fnarg) && fnarg.length);
...
const getSubmissionFeatureCollectionGeoJson = (formPK, IDs, fieldPaths, submitterIds, TSTZRange, reviewStates, deleted, limit) => ({ db }) => {

  const idFilter = hasElements(IDs) ? sql`AND sdef."instanceId" = ANY(${sql.array(IDs, 'text')})` : sql``;
  const formFieldFilter = hasElements(fieldPaths) ? sql`AND ffgeo.path = ANY (${sql.array(fieldPaths, 'text')})` : sql`AND ffgeo.is_default`;
  const doPathInProperties = hasElements(fieldPaths) ? sql`, 'properties', NULL` : sql`, 'properties', json_build_object('fieldpath', path)`;
  const doAggregateByPath = hasElements(fieldPaths) ? sql`` : sql`, path`;
  const submitterFilter = hasElements(submitterIds) ? sql`AND sdef."submitterId" = ANY(${sql.array(submitterIds, 'int4')})` : sql``;
  const TSTZRangeFilter = hasElements(TSTZRange) ? sql`AND sdef."createdAt" <@ tstzrange(${TSTZRange[0]}, ${TSTZRange[1]}, ${TSTZRange[2]})` : sql``;
  const reviewStateFilter = hasElements(reviewStates) ? stateFilterToQueryFragments(['sub', 'reviewState'], reviewStates) : sql``;
...
const getEntityFeatureCollectionGeoJson = (datasetPK, IDs, creatorIds, TSTZRange, conflictStates, deleted, limit) => ({ db }) => {
  ...
  const idFilter = hasElements(IDs) ? sql`AND e.uuid = ANY(${sql.array(IDs, 'text')})` : sql``; // TODO: this should be a uuid[] array once/if the entities."uuid" column is converted from varchar(255) to uuid
  const creatorFilter = hasElements(creatorIds) ? sql`AND e."creatorId" = ANY(${sql.array(creatorIds, 'int4')})` : sql``;
  const TSTZRangeFilter = hasElements(TSTZRange) ? sql`AND e."createdAt" <@ tstzrange(${TSTZRange[0]}, ${TSTZRange[1]}, ${TSTZRange[2]})` : sql``;
  const conflictStatusFilter = hasElements(conflictStates) ? stateFilterToQueryFragments(['e', 'conflict'], conflictStates, 'conflictType') : sql``;
  const deletedFilter = deleted ? sql`IS NOT NULL` : sql`IS NULL`;
  const queryLimit = limit ? sql`LIMIT ${limit}` : sql``;
