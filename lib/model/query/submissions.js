...
const getSelectionEtag = (formPK, odataQuery, orderBy=null, limit=null, offset=null) => ({ db }) => {
  if ((limit || offset) && !orderBy) throw new Error('Passing an ORDER BY sql fragment (or the string "default" to order by event, descending) is mandatory when using pagination');
  const sqlOrderBy = orderBy === null ? sql`` : orderBy === 'default' ? sql`ORDER BY event DESC` : orderBy;
  return db.oneFirst(sql`
    WITH the_slice AS (
      ...
      FROM
          submissions
      WHERE
          submissions."formId" = ${formPK}
          AND
          ${odataFilter(odataQuery, odataToColumnMap)}
          AND
          ${odataExcludeDeleted(odataQuery, odataToColumnMap)}
      ${sqlOrderBy}
      LIMIT ${limit}
      OFFSET ${offset}
    )
