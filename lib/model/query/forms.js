...
const getByEnketoId = (enketoId, xml, version, options = QueryOptions.none, deleted = false) => ({ maybeOne }) => {
  const enketoIdLocation = options.argData.draft === '1' ? 'form_defs.enketoId' : 'forms.enketoId';
  const enketoIdFilterCondition = {};
  enketoIdFilterCondition[enketoIdLocation] = enketoId;
  return _get(maybeOne, options.withCondition(enketoIdFilterCondition), xml, version, deleted);
};
